  ...
  public function saveMess() {
    ...
    if ($value_m->want_email === '1' || $value_m->want_email === '2') {
		...
		if (isset($value_m->full_name)) {
		  ...
		  $message_text = 'Name: '.$value_m->full_name.'<br>';
		}
		else {
		  $headers .= 'From: ' . $value_m->email . ' < ' . $value_m->email . ' > ' . "\r\n";
		}
		if (isset($value_m->phone)) {
		  $message_text = 'Phone: '.$value_m->phone.'<br>';
		}
		$message_text = 'Email: '.$value_m->email.'<br> Contact Preference: '.$cont_pref.'<br>Text: '.$value_m->message_text;
