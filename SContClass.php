...
class SContClass {
    ...
    public function contactScripts() {
        ...
        wp_localize_script('SC_Script', 'contactAjaxUrl', array(admin_url('admin-ajax.php')));
        ...
        if(get_option('twd_captcha') == '2' && $post->post_type === "contact") {
            ...
            wp_localize_script('twd_gcaptcha', 'twd_gcaptcha_key', array($gcaptcha_key));
        }
    ...
    public function add_localization(){
        ...
        if (isset($_GET['page']) && $_GET['page'] == basename(__FILE__) && !$loaded) {
            echo '<div class="error"> Team by 10Web ' . __('Could not load the localization file: ' . $path,$contLDomain ) . '</div>';
            return;
        }
    }
